...
/* TileActions */
TileActions TileActions::singleton;

auto TileActions::GetSingleton(void)->TileActions& {
	return singleton;
}

auto TileActions::GetSingletonConst(void) -> const TileActions& {
	return singleton;
}
...
//Trigger scroll utilities

void TriggerScrollUtilities::FilterGridMotion(/*GridMap**/GridLayer* m, const Rect& r, int* dx, int* dy) {
...
void TriggerScrollUtilities::FilterGridMotionLeft(GridLayer* m, const Rect& r, int* dx, std::list<Point>* crossedTiles) {
	auto x1 = r.x;
	auto x1_next = x1 + *dx;
	if (x1_next < 0)
...
void TriggerScrollUtilities::FilterGridMotionRight(GridLayer* m, const Rect& r, int* dx, std::list<Point>* crossedTiles) {
	auto x2 = r.x + r.w - 1;
	auto x2_next = x2 + *dx;
...
void TriggerScrollUtilities::FilterGridMotionDown(GridLayer* m, const Rect& r, int* dy, std::list<Point>* crossedTiles) {
	auto y1 = r.y + r.h - 1;
	auto y1_next = y1 + *dy;
...
void TriggerScrollUtilities::FilterGridMotionUp(GridLayer* m, const Rect& r, int* dy, std::list<Point>* crossedTiles) {
	auto y2 = r.y;
	auto y2_next = y2 + *dy;
	//auto y1_next = r.y + *dy;
	if (y2_next < 0)
