...
public:
	...
	//todo
	static void FilterGridMotionDown(GridMap* m, const Rect& r, int* dy, std::list<Point>* crossedTiles) {
		auto y1 = r.y + r.h - 1;
		auto y1_next = y1 + *dy;
	...
	//todo
	static void FilterGridMotionRight(GridMap* m, const Rect& r, int* dx, std::list<Point>* crossedTiles) {
		auto x2 = r.x + r.w - 1;
		auto x2_next = x2 + *dx;
